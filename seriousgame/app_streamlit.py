...
# 🌱 **PAGE EMPREINTE CARBONE**
    ...
    if not games:
        ...
        transport = st.number_input("Distance de transport (km)", min_value=0.0, step=0.1, key="transport")

        if st.button("Calculer l'empreinte carbone"):
            carbone = (energie * 0.233) + (materiaux * 2.5) + (transport * 0.1)
            collection.update_one({"_id": ObjectId(game["_id"])}, {"$set": {"carbone": carbone}})
            
            # ✅ Alternative : Message de confirmation sans rechargement
            st.success(f"✅ L'empreinte carbone a été calculée : {carbone:.2f} kgCO₂ (rafraîchissez la page pour voir la mise à jour)")
